...
public class WurstplusArrayList extends WurstplusPinnable {
	...
	public void render() {
		...
		List<WurstplusHack> pretty_modules = Sodaware.get_hack_manager().get_array_active_hacks().stream()
			.sorted(Comparator.comparing(modules -> get(modules.array_detail() == null ? modules.get_tag() : modules.get_tag() + Sodaware.g + " [" + Sodaware.r + modules.array_detail() + Sodaware.g + "]" + Sodaware.r, "width")))
			.collect(Collectors.toList());

		int count = 0;

		if (Sodaware.get_setting_manager().get_setting_with_tag("HUD", "HUDArrayList").in("Top R") || Sodaware.get_setting_manager().get_setting_with_tag("HUD", "HUDArrayList").in("Top L") ) {
		...
		for (WurstplusHack modules : pretty_modules) {
			...
			if (flag) {
				String module_name = (
					...
					modules.get_tag() + Sodaware.g + " [" + Sodaware.r + modules.array_detail() + Sodaware.g + "]" + Sodaware.r
				);

				if (Sodaware.get_setting_manager().get_setting_with_tag("HUD", "HUDArrayList").in("Free")) {
					...
					if (Sodaware.get_setting_manager().get_setting_with_tag("HUD", "HUDArrayList").in("Top R")) {
					...
					if (Sodaware.get_setting_manager().get_setting_with_tag("HUD", "HUDArrayList").in("Top L")) {
						mc.fontRenderer.drawStringWithShadow(module_name, 2, 3 + count * 10, new WurstplusDraw.TravisColor(nl_r,nl_g,nl_b,nl_a).hex());
						count++;
					}
					if (Sodaware.get_setting_manager().get_setting_with_tag("HUD", "HUDArrayList").in("Bottom R")) {
					...
					if (Sodaware.get_setting_manager().get_setting_with_tag("HUD", "HUDArrayList").in("Bottom L")) {
